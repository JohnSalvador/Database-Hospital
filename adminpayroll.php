...
if(empty($fieldarray)) {
	$fieldarray = array(0=>"FirstName AS 'First Name'", 1=>"LastName AS 'Last Name'",2=>"SocialSecurity AS 'Social Security'", 3=>"JobTitle AS 'Job'", 4=>"ProfessionTitle AS 'Profession'");
}
...
<div id="wrapper">
	...
	?>
    <div id="content">
    	<form action="adminpayroll.php" method="POST">
    		Look For: *<br />
            ...
            <table>
                ...
                <tr>
                    ...
                    <td><input type="checkbox" name="Fields[]" value="Wage" checked/> Job</td>
                    <td><input type="checkbox" name="Fields[]" value="ProfessionTitle AS 'Profession'" checked/> Profession</td>
                    <td><input type="checkbox" name="Fields[]" value="BaseSalary AS 'Base Salary'" checked/> Job</td>
		...
		echo '
			<table border="1">
				<tr>
					<th colspan="'.$n.'">Select Employee for PayRoll (click one):</th>
				</tr>
				<tr>';
				$counter=0;
				while($counter<$n) {
		...
		while($row = $result->fetch_assoc()) {
			...
			while($counter<$n){
				if ($counter==0) {
					$id = $row[$fieldname2[$counter]];
				}
				echo '<td><a href="addpayroll.php?id='.$id.'">'.$row[$fieldname2[$counter]].'</a></td>';
				$counter++;
			}
			echo '</tr>';
		}
